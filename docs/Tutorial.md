...
The actual parsing is done when we call `command_line_parser::parse()`. An important thing to notice
...
would have to inspect the `parse()` method's return value in more detail to handle errors manually.
...
As you can see, the `command_line_parser::parse()` method lets us know what's wrong (we didn't
...
`description()` method when using the `parser_builder`.
...
remaining arguments won't be parsed and the `parse()` method returns a failure result, so the
...
`mode()` to `parsing_mode::long_short`, and we've made argument names case sensitive. We've also
...
arguments, by calling `short_name()`. This gives them a short name using the first character of
their long name, so `-m` and `-i` in this case. You can also specify a custom short name by
supplying a parameter to the `short_name()` method.

If you want an argument to only have a short name, there are overloads of `add_argument()` that
take a short name.
...
`mode()` and `case_sensitive()`; while we could do that here, we probably want to have those options
...
The call to `description()` sets a description for the application as a whole, rather than any
...
common to each command, such as the parsing mode in this case (`case_sensitive` isn't needed because
...
After the manager is set up, we can run a command. The `run_command()` method takes the provided
...
> `create_command()` that you can call to do this manually.
...
`add_multi_value_argument()` method, which means it can have multiple values by supplying it
