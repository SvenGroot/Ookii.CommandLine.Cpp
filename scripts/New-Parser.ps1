...
process {
    ...
    foreach ($file in $inputs)
    {
        ...
        foreach ($info in (Convert-Arguments $contents $context)) {
            $infoCount += 1
            $result += "ookii::basic_parser_builder<$($context.CharType)> $($info.TypeName)::create_builder(std::basic_string<$($context.CharType)> command_name, ookii::basic_localized_string_provider<$($context.CharType)> *string_provider, const std::locale &locale)
{
